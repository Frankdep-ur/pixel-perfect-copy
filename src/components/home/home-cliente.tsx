...
export function HomeCliente({ nome, reserva }: { nome: string; reserva: ReservaHome }) {
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-4 px-4 py-5 md:max-w-2xl lg:max-w-5xl">
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
          ...
          <button
            ...
            onClick={() => {
              ...
              window.open(
                linkSuporte(`Olá! Quero alterar a reserva ${codigo}.`),
                "_blank",
                "noopener,noreferrer",
              );
            }}
          ...
          <dl className="space-y-1.5 text-[13px]">
            ...
            {Number(reserva.valor_seguro ?? 0) > 0 && (
              <div className="flex items-baseline justify-between gap-3">
                ...
                <dd className="text-foreground">{formatBRL(Number(reserva.valor_seguro))}</dd>
              </div>
            )}
      ...
      {cumpridas >= 4 && (
        <a
          href={linkSuporte(`Olá! Quero registrar a avaliação da reserva ${codigo}.`)}
          target="_blank"
          rel="noreferrer"
          className="flex min-h-14 items-center justify-center rounded-[24px] bg-accent text-[15px] font-bold text-background"
        >
